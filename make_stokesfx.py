	...
	for i in enumerate(baseline):
		...
		for j in enumerate(baseline):
			...
			if baseline_1 != baseline_2:
				...
				plt.ylabel('LST')
				plt.subplot(143)
				plt.imshow(np.log10(np.abs(d_yx1[(ant_i1, ant_j1)]['yx']))-np.log10(np.abs(d_xy2[(ant_i2, ant_j2)]['yx'])), aspect='auto', vmax=0, vmin=-6, cmap='viridis')
				plt.title('yx Visibility')
				plt.xlabel('Frequency bin')
				plt.ylabel('LST')
				plt.subplot(144)
				plt.imshow(np.log10(np.abs(d_yy1[(ant_i1, ant_j1)]['yy']))-np.log10(np.abs(d_xy2[(ant_i2, ant_j2)]['yx'])), aspect='auto', vmax=0, vmin=-6, cmap='viridis')
