...
def avgfreqall(data_dir):
	...
	for i in range(len(xx_data)):
		...
		for elem,antstr in enumerate(antlist_all):
			...
			if ('%s' %(antstr) not in avgstokes_dict):
				...
				avgstokes_dict['%s' %(antstr)]['visxx_real'] = np.zeros((vis_xx.shape[1]))
				avgstokes_dict['%s' %(antstr)]['visxx_imag'] = np.zeros((vis_xx.shape[1]))
				avgstokes_dict['%s' %(antstr)]['visyy_real'] = np.zeros((vis_xx.shape[1]))
				avgstokes_dict['%s' %(antstr)]['visyy_imag'] = np.zeros((vis_xx.shape[1]))
			
				for it in range(vis_xx.shape[0]):
					avgstokes_dict['%s' %(antstr)]['visxx_real'] += vis_xx_real[it,:]
					avgstokes_dict['%s' %(antstr)]['visxx_imag'] += vis_xx_imag[it,:]
					avgstokes_dict['%s' %(antstr)]['visyy_real'] += vis_yy_real[it,:]
					avgstokes_dict['%s' %(antstr)]['visyy_imag'] += vis_yy_imag[it,:]
	...
	for elem,antstr in enumerate(antlist_all):    
		avgstokes_dict['%s' %(antstr)]['visxx_real'] /= n_avg
		avgstokes_dict['%s' %(antstr)]['visxx_imag'] /= n_avg
		avgstokes_dict['%s' %(antstr)]['visyy_real'] /= n_avg
		avgstokes_dict['%s' %(antstr)]['visyy_imag'] /= n_avg
...
def avgfreqcalc(data_dir, antstr, visibility):
