...
def viscalculator(data_dir):
	...
	for i, element in enumerate(baselineiterator):
			ax1.plot(np.imag(xxtotalavg[:, i]))
	ax1.plot(np.imag(vis_xx), 'g-', linewidth=3, label="hd5line")
	ax1.legend()
	ax2 = plt.subplot(412)
	ax2.set_title("Vis YY")
	ax2.set_ylabel("Average Power")
	for i, element in enumerate(baselineiterator):
			ax2.plot(np.imag(yytotalavg[:, i]))
	ax2.plot(np.imag(vis_yy), 'g-', linewidth=3, label="hd5line")
	ax2.set_ylim(-0.1, 0.1)
	ax3 = plt.subplot(413)
	ax3.set_title("Vis XY")
	ax3.set_ylabel("Average Power")
	for i, element in enumerate(baselineiterator):
			ax3.plot(np.imag(xytotalavg[:, i]))
	ax3.plot(np.imag(vis_xy), 'g-', linewidth=3, label="hd5line")
	...
	for i, element in enumerate(baselineiterator):
			ax4.plot(np.imag(yxtotalavg[:, i]))
	ax4.plot(np.imag(vis_yx), 'g-', linewidth=3, label="hd5line")
